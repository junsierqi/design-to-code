...
class ValidateDesignToCodeTests(unittest.TestCase):
    ...
    def test_install_skill_ignores_generated_files_for_parity(self) -> None:
        script = REPO_ROOT / "scripts" / "install_skill.py"
        generated_dir = REPO_ROOT / "skills" / "design-to-code" / "scripts" / "__pycache__"
        generated_file = generated_dir / "install_parity_pollution.cpython-313.pyc"
        generated_dir.mkdir(exist_ok=True)
        generated_file.write_bytes(b"generated bytecode pollution")
        try:
            with tempfile.TemporaryDirectory() as tmp:
                target = Path(tmp) / "design-to-code"
                subprocess.run(
                ...
                self.assertIn("parity ok:", result.stdout)
                self.assertFalse((target / "scripts" / "__pycache__" / generated_file.name).exists())
        finally:
